...
wp() {
  compose run --rm wpcli-kb wp "$@"
}

wait_for_wordpress() {
  local attempts=60
  until compose exec -T wordpress-kb test -f /var/www/html/wp-config.php >/dev/null 2>&1; do
    attempts=$((attempts - 1))
...
fix_permissions() {
  compose exec -T -u root wordpress-kb sh -lc '
...
ensure_site() {
  ...
  wp option update home "${kb_public_url}"
  wp option update siteurl "${kb_public_url}"
...
app_password() {
  local output="secrets/kb-application-password.txt"
  local generated
  if [[ -f "$output" ]]; then
    echo "Application password already exists in $output"
  else
    generated="$(wp user application-password create "${WP_PUBLISHER_USER}" "Codex API $(date +%Y-%m-%d)" --porcelain)"
    printf '%s\n' "${generated//[[:space:]]/}" > "$output"
    chmod 600 "$output"
    echo "Saved application password to $output"
  fi

  write_app_password_to_env "$(cat "$output")"
}

echo "Starting production personal knowledge base..."
compose up -d db-kb wordpress-kb caddy
wait_for_wordpress
fix_permissions
...
echo
echo "Production personal knowledge base is ready:"
echo "  ${kb_public_url}/wp-login.php"
echo
echo "Admin:"
echo "  user: ${WP_ADMIN_USER}"
echo "  password: stored in .env as WP_ADMIN_PASSWORD"
echo
echo "API publishing:"
